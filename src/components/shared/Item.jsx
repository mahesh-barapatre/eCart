...
function Item({title, price, image, func, Id}) {
  ...
  return (
    <div 
    className='bg-blue-50 m-3 p-3 mb-2 h-60 w-40 flex justify-between hover:scale-105 transition ease-in-out duration-500 flex-col'>
        <img 
        onClick={()=>{
          dispatch(setItem(Id));
          navigate("/item")
      ...
      <div className='h-5 w-full flex justify-around items-center'>
        ...
        <button 
        onClick={func}
        className='text-white w-1/2 p-1 text-sm bg-black'>ADD</button>
      </div>     
